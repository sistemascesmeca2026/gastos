...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.clave || body.nombre) {
      await pool.query(
        `UPDATE funciones SET clave = COALESCE($1, clave), nombre = COALESCE($2, nombre) WHERE id = $3`,
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (err.code === '23503') {
      return NextResponse.json({ error: 'No se puede eliminar: esta función tiene capítulos o partidas con movimientos registrados' }, { status: 409 });
    }
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
