-- Agrega el cálculo de "Modificado real" a la vista v_saldo_partida:
-- Modificado real = Modificado del último corte oficial + transferencias de
-- entrada - transferencias de salida. Antes, "Modificado" se mostraba fijo
-- tal como venía del corte, sin reflejar las adecuaciones/transferencias
-- capturadas después. No se elimina la columna "modificado" original
-- (el valor crudo del corte); se agrega "modificado_real" como columna nueva.
-- No modifica ningún dato en movimientos ni linea_base.
...
CREATE OR REPLACE VIEW v_saldo_partida AS
 WITH ultima_linea_base AS (
         ...
         SELECT movimientos.partida_id,
            ...
            COALESCE(sum(movimientos.monto) FILTER (WHERE movimientos.tipo_tramite = 'transferencia_salida'::tipo_tramite), 0::numeric) AS transferido_salida_real
           FROM movimientos
 ...
 SELECT p.id AS partida_id,
    ...
    COALESCE(lb.modificado, 0::numeric)
      + COALESCE(a.transferido_entrada_real, 0::numeric)
      - COALESCE(a.transferido_salida_real, 0::numeric) AS modificado_real
   FROM partidas p
